...
$appVersion = & $python -c 'from desktop_notes import __version__; print(__version__)'
if ($LASTEXITCODE -ne 0 -or -not $appVersion) {
    throw 'Unable to read the application version.'
}
$appVersion = $appVersion.Trim()
$msixVersion = if ($PackageVersion) { $PackageVersion.Trim() } else { "$appVersion.0" }
if ($msixVersion -notmatch '^\d+\.\d+\.\d+\.\d+$') {
    throw "Invalid MSIX package version: $msixVersion"
}
...
if (-not $makeAppx -or -not $makePri) {
    throw 'Microsoft Windows SDK packaging tools were not found.'
}
...
$outputPackage = Join-Path $outputDirectory "BittyNote-$msixVersion-x64.msix"
...
Write-Host "Store MSIX package completed: $outputPackage"
